...
def get_rho_data(H,site):
    ...
    obs= np.loadtxt(r'./icecores/dens_%s.txt'%(site))
...
def smooth_raw_data(z_raw, rho_raw, H):
...
#surface values from breant
surface_rhos={'dye3':357.0,    'grip':367.0,   'neem':307.2,   'ngrip':299.9,   'site_2':350.1,     'siteA_crete':321.7}
...
for i in range(len(sites)):
    ...
    z_obs_smooth,rho_obs_smooth=smooth_raw_data(z_obs_raw,rho_obs_raw,H)

    print(f'{rho_surf=} ------{rho_obs_smooth[0]=} {rho_obs_smooth[-1]=}')
    ...
    for j in range(len(Ks)):
        ...
        rho_model,z_rho_model=solve_forward_problem(H,Hres,acc_rate,Ks[j],n,Aglen,rho_surf,
                                                    rho_ice=916,phi_snow=rho_surf/rho_ice) #<<<changes per site
...
ax.plot(Ks,RMSE_08[0,:],c='#0072B2',linewidth=2,label='DYE3')
ax.plot(Ks,RMSE_08[1,:],c='#56B4E9',linewidth=2,label='GRIP')
ax.plot(Ks,RMSE_08[2,:],c='#009E73',linewidth=2,label='NEEM')
ax.plot(Ks,RMSE_08[3,:],c='#E69F00',linewidth=2,label='NGRIP')
ax.plot(Ks,RMSE_08[4,:],c='#D55E00',linewidth=2,label='SITE-2')
ax.plot(Ks,RMSE_08[5,:],c='#CC79A7',linewidth=2,label='SITE-A')


ax.set_xlim([0,1000])
ax.set_ylim([0,55])

ax.set_xlabel(r'$k$',family='sans serif')#,labelpad=10)
ax.set_ylabel(r'RMSE  (kg m$^{-3}$)',family='sans serif',fontsize=15,labelpad=7)
...
ax.legend(ncol=3,loc='upper right',prop={'family':'sans serif', 'size':12})#, frameon=False)
# plt.title('RMSE top 40m')

plt.savefig('RMSE_site_by_site_rhohat08.png',dpi=150,format='png', bbox_inches='tight')
