...
def get_Aglen(U,heated_margins=False):
    ...
    # Parameters to calculate the flow-rate factor:
    R = Constant(8.314)
    #the following are only applicable for temperatures below -10ºC, but we are staying well below that
    #(even at the shear margins)
    Q = Constant(60e3)
    A0 = Constant(3.985e-13)
...
K=int(input('\nChoose value of K? (10-2000, press enter for default=2000): ') or 2000)

heated_margins=input('\nDo you want to consider the shear margins to be hotter? (write True or False): ')
...
print(f'\n----CHOSEN OPTIONS---{K=}  {heated_margins=}  {margintag=}\n')
...
Hres,Lres=100,200                  #Nodal resolution.-->(Paper simulations run with Hres=150 and Lres=300)

rho_surf=290                       #Density of snow at the surface (kg/m^3)
...
phi_snow=0.4                       #relative density in coefficient functions a and b
...
acc_rate= 0.448/yr2s #m_snow_eq/yr karlson 2020
print(f'{acc_rate=} m_snow_eq/s {acc_rate*yr2s=} m_snow_eq/yr')

g=Constant((0.0,-9.82)) 

dtf=1.0 #dt prefactor
...
#computed from Greenland's surface velocity map (Measures program)
epsyy=np.load("data/NEGIS_epsyy.npy")
epsxy=np.load("data/NEGIS_epsxy.npy")
...
sfcmin=np.array([H])
simtime=np.array([0])
dHdts=np.array([np.nan]) #placeholder
...
dHdt_tol=0.035       #Change of height per unit time to consider that we have reached the steady state
...
while dHdt>=dHdt_tol:
    ...
    deg_v=2
    V=VectorFunctionSpace(mesh, "Lagrange", deg_v)
    ...
    #---logging
    sfcmax=np.append(sfcmax,np.max(sfc.zvec))
    sfcmin=np.append(sfcmin,np.min(sfc.zvec))
    simtime=np.append(simtime,(tstep+1)*dt/yr2s) #because it starts from 0
    dHdts = np.append(dHdts, dHdt)

    if tstep%10==0:

        plt.figure()
        plt.plot(simtime,sfcmax,c='r',label='Surface max height')
        plt.plot(simtime,sfcmin,c='b',label='Surface min height')
        plt.xlabel('Simulation time (yr)')
        plt.ylabel('Surface height (m)')
        plt.legend()
        plt.savefig(outfolder+'SurfaceHminmax_K_'+str(K)+'_'+margintag+f'.png',format='png',dpi=200)
        plt.close()

        plt.figure()
        plt.plot(simtime,sfcmax-sfcmin,label='Max shear margin trough depth')
        plt.plot(simtime,dHdts,label='dHdt\n(max surface height difference\nw.r.t. previous iteration)')
        plt.axhline(dHdt_tol,ls='--',c='tab:orange',label='dHdt_tol')
        plt.xlabel('Simulation time (yr)')
        plt.ylabel('(m)')
        plt.legend()
        plt.savefig(outfolder+'SurfacedH_K_'+str(K)+'_'+margintag+f'.png',format='png',dpi=200)
        plt.close()



    #to give a (very rough) estimate of at least how much time it will take to finish
    tnow=time.time()
    print(f'\n{tstep=}------t={np.round(tstep*dt/yr2s,3)}yrs------>dHdt={np.round(dHdt,3)}/{dHdt_tol}m')
    print(f'               iter_dt={np.round(tnow-tprev,2)}s \n           (expected_minimum_time={np.round(np.ceil((dHdt-dHdt_tol)/(dHdtprev-dHdt))*(tnow-tprev)/60,2)} mins ---will be considerably more)\n ')
...
plt.savefig(outfolder+'strain_softening_SS_rho_K_'+str(K)+'_'+margintag+f'.png',format='png',dpi=200)
...
ax.set_aspect(aspect=125)
ax.set_ylabel('Depht (m)')
plt.savefig(outfolder+'strain_softening_V_SS_K_'+str(K)+'_'+margintag+f'.png',format='png',dpi=200)
...
np.save(outfolder+'modeloutput_surface_elevation_K_'+str(K)+'_'+margintag+f'.npy',zs_nodes[:,-1])
np.save(outfolder+'modeloutput_nodeZs_K_'+str(K)+'_'+margintag+f'.npy',zs_nodes)
np.save(outfolder+'modeloutput_columnXs_K_'+str(K)+'_'+margintag+f'.npy',xs_nodes)
...
np.save(outfolder+'modeloutput_rhos_K_'+str(K)+'_'+margintag+f'.npy',rho_nodes)
np.save(outfolder+'modeloutput_BCOdepth_K_'+str(K)+'_'+margintag+f'.npy',zsBCO)
...
plt.ylabel('Depth (m)')
plt.savefig(outfolder+'NEGIS_surface_K_'+str(K)+'_'+margintag+f'.png',dpi=150,format='png')
